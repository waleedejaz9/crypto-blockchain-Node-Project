...
async function checkTxId(tx, payload) {
    return new Promise(async (resolve, reject) => {
        if (tx.result) {
            ...
            const txIdAsset = await multichain.transferAsset(payload.sellerAddress, payload.buyerAddress, payload.name, payload.quantity, payload.price)
            if (tx.result) {
        ...
        db.collection("AssetExchangeData").find({
            ...
            if (result[0]) {
                if (result[0].walletId === walletID && result[0].promotionPrice == promotionPrice) {
                    ...
                    notification(Notification, "b5dd85e4-e6c2-4d4e-818e-ad72e27461b5", walletID)
                    resolve({
                        success: true,
...
router.post('/postAssetRequest', VerifyToken, async function (req, res, next) {
    ...
    User.findOne({
        ...
        if (user) {
            ...
            if (walletId == "b5dd85e4-e6c2-4d4e-818e-ad72e27461b5") {
                status = "Accepted";
            ...
            if (user.walletId !== "b5dd85e4-e6c2-4d4e-818e-ad72e27461b5") {
                notification(Notification, user.walletId, "b5dd85e4-e6c2-4d4e-818e-ad72e27461b5")
            }
            res.send({
                success: true,
                    ...
                    if (result[0].walletId !== user.walletId) {
                        ...
                        db.collection("AssetExchangeData").find(query).toArray(async (err, result) => {
                            ...
                            if (result[0]) {
                                ...
                                let toAddress = "1A9bPEvqoscEGP2NPwBHscSB4DWujcjdxrjZYm";
                                ...
                                if (tx.result) {
                                    ...
                                    notification(userNotification, user.walletId, 'b5dd85e4-e6c2-4d4e-818e-ad72e27461b5');
                                    db.collection('AssetExchangeData').update({
                                        "id": id
                                    }, {
                                        $set: {
                                    ...
                                    notification(adminNotification, 'b5dd85e4-e6c2-4d4e-818e-ad72e27461b5', user.walletId);
                                    res.send({
                                        success: true,
...
router.post("/updatePromotionStatusBTC", VerifyToken, async (req, res, next) => {
    const id = req.body.id;
    ...
    if (txHash) {
        db.collection("AssetExchangeData").find({
            ...
            if (result[0]) {
                ...
                res.send({
                    success: true,
                    message: "Pormotion Status has enabled!"
                })

            } else {
                res.send({
                    success: false,
                    message: "No post available against this id!"
                })
            }
        ...
        res.send({success:false,message:"Please provide transaction Hash!"})
    }
})
